...
namespace Math
{
    ...
    {
    public:
        D3D12PLAYGROUNDCOMMON_API RandomNumberGenerator(uint32_t s = 0) : m_gen(m_rd())
        {
            if (s != 0)
                SetSeed(s);
        }
    ...
    extern D3D12PLAYGROUNDCOMMON_API RandomNumberGenerator g_RNG;
} // namespace Math
}