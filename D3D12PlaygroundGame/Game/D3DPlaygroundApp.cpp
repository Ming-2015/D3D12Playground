...
#include "pch.h"
#include <D3D12PlaygroundGraphics/Public/Core/BufferManager.h>
#include <D3D12PlaygroundGraphics/Public/Core/CameraController.h>
#include <D3D12PlaygroundGraphics/Public/Core/Camera.h>
#include <D3D12PlaygroundGraphics/Public/Core/CommandContext.h>
#include <D3D12PlaygroundGraphics/Public/Core/DepthOfField.h>
#include <D3D12PlaygroundGraphics/Public/Core/Display.h>
#include <D3D12PlaygroundGraphics/Public/Core/FXAA.h>
#include <D3D12PlaygroundGraphics/Public/Core/GameCore.h>
#include <D3D12PlaygroundGraphics/Public/Core/GameInput.h>
#include <D3D12PlaygroundGraphics/Public/Core/MotionBlur.h>
#include <D3D12PlaygroundGraphics/Public/Core/ParticleEffectManager.h>
#include <D3D12PlaygroundGraphics/Public/Core/PostEffects.h>
#include <D3D12PlaygroundGraphics/Public/Core/TemporalEffects.h>
#include <D3D12PlaygroundGraphics/Public/Core/SSAO.h>
#include <D3D12PlaygroundGraphics/Public/Core/SystemTime.h>
#include <D3D12PlaygroundGraphics/Public/Core/TextRenderer.h>
#include <D3D12PlaygroundGraphics/Public/Core/TextureManager.h>
#include <D3D12PlaygroundGraphics/Public/Core/ShadowCamera.h>

#include <D3D12PlaygroundGraphics/Public/Engine/SponzaRenderer.h>
#include <D3D12PlaygroundGraphics/Public/Engine/glTF.h>
#include <D3D12PlaygroundGraphics/Public/Engine/Renderer.h>
#include <D3D12PlaygroundGraphics/Public/Engine/Model.h>
#include <D3D12PlaygroundGraphics/Public/Engine/ModelLoader.h>
...
void LoadIBLTextures()
{
    ...
    WIN32_FIND_DATA ffd;
    HANDLE hFind = FindFirstFile(L"Assets/Textures/*_diffuseIBL.dds", &ffd);
    ...
    if (hFind != INVALID_HANDLE_VALUE) do
    {
        ...
        TextureRef diffuseTex = TextureManager::LoadDDSFromFile(L"Assets/Textures/" + diffuseFile);
        if (diffuseTex.IsValid())
        {
            TextureRef specularTex = TextureManager::LoadDDSFromFile(L"Assets/Textures/" + specularFile);
...
#else
        m_ModelInst = Renderer::LoadModel(L"Assets/Sponza/PBR/sponza2.gltf", forceRebuild);
...
void D3D12PlaygroundApp::RenderScene()
{
    ...
    else
    {
        ...
        m_SunShadowCamera.UpdateMatrix(-SunDirection, Vector3(0, -500.0f, 0), Vector3(5000, 3000, 3000),
            (uint32_t)Graphics::g_ShadowBuffer.GetWidth(), (uint32_t)Graphics::g_ShadowBuffer.GetHeight(), 16);
        ...
        sorter.SetDepthStencilTarget(Graphics::g_SceneDepthBuffer);
        sorter.AddRenderTarget(Graphics::g_SceneColorBuffer);
        ...
        if (!SSAO::DebugDraw)
        {
...

            {
                ...
                shadowSorter.SetDepthStencilTarget(Graphics::g_ShadowBuffer);
...

            {
                ...
                gfxContext.TransitionResource(Graphics::g_SSAOFullScreen, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
                gfxContext.TransitionResource(Graphics::g_SceneDepthBuffer, D3D12_RESOURCE_STATE_DEPTH_READ);
    ...
    ParticleEffectManager::Render(gfxContext, m_Camera, Graphics::g_SceneColorBuffer, Graphics::g_SceneDepthBuffer, Graphics::g_LinearDepth[FrameIndex]);

    // Until I work out how to couple these two, it's "either-or".
    if (DepthOfField::Enable)
        DepthOfField::Render(gfxContext, m_Camera.GetNearClip(), m_Camera.GetFarClip());
    else
        MotionBlur::RenderObjectBlur(gfxContext, Graphics::g_VelocityBuffer);
